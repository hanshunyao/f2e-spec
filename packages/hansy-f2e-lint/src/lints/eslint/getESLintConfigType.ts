 ...
 * @param cwd
 * @param pkg
 * @returns hansy-f2e-eslint-config/index
 * @returns hansy-f2e-eslint-config/react
 * @returns hansy-f2e-eslint-config/typescript/index
 * @returns hansy-f2e-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'hansy-f2e-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
